...
const TodaySchedule: React.FC = () => {
  ...
  const [showImportAttendanceDialog, setShowImportAttendanceDialog] =
    useState(false);
    ...
    timeSlots
      ...
      .forEach((timeSlot) => {
        const daySlot = daySlots.find(
          (ds) => ds.timeSlotId === timeSlot.id && ds.day === selectedDayName
        );
        if (!daySlot) return;
            ...
            {termSettings && (
              <div className="text-sm text-muted-foreground whitespace-nowrap">
                <span className="font-medium">
                  Term: {format(parseISO(termSettings.startDate), "MMM dd")} -{" "}
              ...
              {specialDate ? (
                <div className="text-center py-8">
                  <div className="text-muted-foreground">
                    No schedule -{" "}
                    {specialDate.type === "holiday" ? "Holiday" : "Exam Day"}
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  {!termSettings && (
                    <div className="text-sm text-muted-foreground mt-2">
                      Term settings not configured. Please set term dates in
                      Term Settings.
                    </div>
                  )}
                </div>
                    ...
                    <>
                      {scheduledLectures.map((lecture, index) => (
                        <TodayScheduleItem
                          ...
                          attendance={todayAttendance.find(
                            (a) => a.timeSlotId === lecture.timeSlot.id
                          )}
                          isExtraClass={false}
                        />
                      ))}
                      ...
                      {todayExtraClasses.map((extraClass) => {
                        const timeSlot = timeSlots.find(
                          (ts) => ts.id === extraClass.timeSlotId
                        );
                        const subject = subjects.find(
                          (s) => s.id === extraClass.subjectId
                        );
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  ...
                  {!dateInTerm && (
                    <Badge variant="secondary">Outside Term</Badge>
                  )}
                </div>
