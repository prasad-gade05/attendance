...
import { format, parseISO, eachDayOfInterval, isWithinInterval, getDay } from 'date-fns'
...
const AttendanceStatsPanel: React.FC<AttendanceStatsPanelProps> = ({
  ...
  const getTotalStats = () => {
    return detailedStats.reduce((total, stat) => ({
  ...
  const overallPercentage = totalStats.totalLectures > 0 
    ? Math.round((totalStats.attendedLectures / totalStats.totalLectures) * 100)
    : 0
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        {!termSettings ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground mt-4">Calculating statistics...</p>
          </div>
        ) : (
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={overallPercentage >= 75 ? 'default' : 'destructive'}>
            ...
            <TabsContent value="by-subject" className="space-y-4">
              {detailedStats.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {detailedStats
                    .sort((a, b) => getAttendancePercentage(b) - getAttendancePercentage(a))
                    .map((stat) => {
                        ...
                        <Card key={stat.subjectId}>
                          ...
                          <CardContent>
                            ...
                            <div className="grid grid-cols-4 gap-4 text-sm">
                              <div className="text-center">
                                <div className="font-bold text-blue-600">{stat.totalLectures}</div>
                                <div className="text-muted-foreground">Total</div>
                              </div>
                              <div className="text-center">
                                <div className="font-bold text-green-600">{stat.attendedLectures}</div>
                                <div className="text-muted-foreground">Attended</div>
                              </div>
                              <div className="text-center">
                                <div className="font-bold text-red-600">{stat.missedLectures}</div>
                                <div className="text-muted-foreground">Missed</div>
                              </div>
                              <div className="text-center">
                                <div className="font-bold text-yellow-600">{stat.cancelledLectures}</div>
                                <div className="text-muted-foreground">Cancelled</div>
                              </div>
                            </div>
                          </CardContent>
